...
module.exports = function() {
	...
	return function(req, res, next) {
		...
		if (authHeader) {
			...
			if (requestPath === "/version" || requestPath === "/status" || requestPath === "/catalog"){
				next();
			} else {
				return res.status(401).json({ message: 'An invalid authorization header was passed in.'});
			}
...
function isAuthenticated(bearerToken, callback) {
    ...
    if(!bearerToken) {
        logger.error('Looking up the user profile failed due to an ' +
	...
	return tiamClient.getWhoami(bearerToken, function(err, r) {
        if (err) {
            logger.error('Looking up the user profile from ' +
