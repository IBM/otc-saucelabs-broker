...
exports.getWhoami = function(token, callback) {
    ...
    request(options, function(error, res, body) {
        ...
        switch (res.statusCode) {
            ...
            case 400 :
                logger.error('Looking up the TIAM failed due to an invalid ' +
            ...
            case 401:
                logger.error('Looking up the TIAM failed due to an unauthorized ' +
                            'request. Error: ' + body.error);
                return callback(body.error, 401);
            case 403:
                logger.error('Looking up the TIAM failed due to a forbidden ' +
                            'request. Error: ' + body.error);
            ...
            case 500:
                logger.error('Looking up the TIAM failed due to an internal ' +
